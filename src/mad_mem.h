...
#define mymalloc_atomic(fn, sz)     myptrchk("A", fn, sz, GC_MALLOC_ATOMIC_IGNORE_OFF_PAGE(sz))
...
#define myrecalloc(fn, p, osz, sz)  ((void*)((char*)memset((char*)myptrchk("C",fn,sz,GC_REALLOC((p),(sz)))+(osz),0,(sz)-(osz))-(osz)))
...
#define mymalloc_atomic(fn, sz)     myptrchk("A", fn, sz, malloc(sz))
#define myrealloc(fn, p, sz)        myptrchk("R", fn, sz, realloc((p),(sz)))
#define myfree(fn, p)               ((void)(free(myptrchk("F", fn, 0, p)), (p)=0))

#define mycalloc(fn, n, sz)         myptrchk("M", fn, sz, calloc((n),(sz)))
#define mycalloc_atomic(fn, n, sz)  myptrchk("A", fn, sz, calloc((n),(sz)))
#define myrecalloc(fn, p, osz, sz)  ((void*)((char*)memset((char*)myptrchk("C",fn,sz,realloc((p),(sz)))+(osz),0,(sz)-(osz))-(osz)))
